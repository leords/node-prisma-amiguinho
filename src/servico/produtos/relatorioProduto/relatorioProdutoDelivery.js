...
export const RelatorioProdutoDelivery = async (inicio, fim, vendedor, produtoId) => {

    const totais = await prismaCliente.itemPedidoDelivery.aggregate({
                        where: {
                            produtoId: produtoId,
                            pedido: {
                                vendedor: vendedor,
                    ...
                    const pedidoPositavo = await prismaCliente.itemPedidoDelivery.groupBy({
                        by: ['pedidoId'],
                        where: {
                            produtoId: produtoId,
                            pedido: {
                    ...
                    const totalPedidos = await prismaCliente.pedidoDelivery.count({
                        where: {
                            vendedor: vendedor,
                            data: {
                                gte: inicio,
