...
class DeliveryServico {
  executar(dados) {
    ...
    const pedidosEntregues = this.#filtrarPorStatus(pedidos, 'entregue')

    const total = pedidoHelper.total(pedidosEntregues)

    const quantidadePedidos = pedidoHelper.quantidade(pedidosEntregues)

    const ticketMedio = pedidoHelper.ticketMedio(pedidosEntregues)

    const formasPagamento = this.#buscarFormasPagamento(pedidosEntregues, total)

    const status = {
      pendentes: this.#quantidadeStatus(pedidos, 'pendente'),

      carregados: this.#quantidadeStatus(pedidos, 'carregado'),

      entregues: this.#quantidadeStatus(pedidos, 'entregue'),

      cancelados: this.#quantidadeStatus(pedidos, 'cancelado'),
    }

    const tempoEntrega = this.#tempoMedioEntrega(pedidosEntregues)
  ...
  #tempoMedioEntrega(pedidos) {
    ...
    for (const pedido of pedidos) {
      if (!pedido.dataCarregada || !pedido.dataEntrega) {
        continue
      }
