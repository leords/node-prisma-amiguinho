...
class BalcaoServico {
  executar(dados) {
    ...
    const formasPagamento = this.#buscarFormasPagamento(pedidos, total)
    ...
    const vendedores = Object.entries(vendedoresAgrupados).map(
      ([nome, pedidosVendedor]) => {
        const totalVendedor = pedidoHelper.total(pedidosVendedor)

        const quantidade = pedidoHelper.quantidade(pedidosVendedor)

        const ticket = pedidoHelper.ticketMedio(pedidosVendedor)

        const valeInterno = this.#buscarValeInterno(pedidosVendedor)
        ...
        return {
          ...
          pedidos: quantidade,
          ticketMedio: ticket,
          participacao: this.#percentual(totalVendedor, total),
          valeInterno,
        }
      }
    )
